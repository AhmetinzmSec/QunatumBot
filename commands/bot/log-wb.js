...
exports.run = (client, message, args) => {
    ...
    if (args[0] === "ayarla" || args[0] == "set") {
        ...
        db.set("welbye" + message.guild.id, channel.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik Kazanıldı')
            .addField(`Sunucunun log kanalı ${channel.name} oldu`, 'Artık giriş-çıkış kaydı bu kanala mesajlı bildirim şeklinde tutulacak')
            .setColor(renk)
            .setFooter(slogan)
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("welbye" + message.guild.id)) return message.channel.send("Sistem zaten kapalı")

        db.delete("welbye" + message.guild.id)

        const embed = new MessageEmbed()
            .setTitle('İşlevlik İptal Edildi')
            .addField(`Sunucunun log kanalı silindi`, 'Artık giriş çıkış kaydı tutulmayacak')
            .setColor(renk)
            .setFooter(slogan)
        message.channel.send(embed)
    }
