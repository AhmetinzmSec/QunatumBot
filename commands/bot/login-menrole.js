...
exports.run = (client, message, args) => {
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("erkek" + message.guild.id)) return message.reply(rolyoh)

        db.delete("erkek" + message.guild.id)

        const sifirlanmis = new MessageEmbed()

            .setTitle('Erkek Rolü Sıfırlandı')
    ...
    if (rol) {
        ...
        const oldula = new MessageEmbed()
            ...
            .setDescription(`**${message.author} Sunucumuza kayıt olan üyeler erkek olarak kayıt olduğunda artık <@${rol.id}> rolü verilecek**`)
            .setColor(renk)
            .setFooter(slogan)
