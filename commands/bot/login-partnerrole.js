...
exports.run = (client, message, args) => {
    ...
    const izinyok = new MessageEmbed()
        ...
        .setDescription('**Bu komutu kullanmak için `ROLLERİ_YÖNET` ya da `YÖNETİCİ` yetkisine sahip olmalısın**')
        .setColor(renk)
        .setFooter(slogan)
    ...
    if (args[0] === "sil" || args[0] == "delete") {
        ...
        if (!db.has("partner" + message.guild.id)) return message.reply(rolyoh)

        db.delete("partner" + message.guild.id)

        const sifirlanmis = new MessageEmbed()

            .setTitle('Partner Rolü Sıfırlandı')
    ...
    if (rol) {
        ...
        db.set("partner" + message.guild.id, rol.id)

        const oldula = new MessageEmbed()

            .setTitle('Partner Rolü Ayarlandı')
            .setDescription(`**${message.author} Sunucumuza kayıt olan üyeler partner kanalını görüntülemek istediğinde olduğunda artık <@${rol.id}> rolü verilecek**`)
            .setColor(renk)
            .setFooter(slogan)
