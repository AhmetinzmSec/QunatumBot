...
exports.run = async (client, message, args) => { /// Created by ByMayFe_0#3960 && Zeaste#2841 for CodAre
    ...
    var logkanali = db.fetch("rlog" + message.guild.id)  // LOGKANALİİD

    var rolcukE = db.fetch("erkek" + message.guild.id) // ERKEK ROLÜ İD
    var rolcukK = db.fetch("kadin" + message.guild.id) // KIZ ROLÜ İD
